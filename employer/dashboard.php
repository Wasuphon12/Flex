...
$jobsStmt = $pdo->prepare("SELECT j.job_id AS id,j.job_title AS title,jc.category_slug AS job_type,j.pay_amount,j.pay_unit,j.job_status AS status,COUNT(a.application_id) applicants FROM jobs j JOIN job_categories jc ON jc.job_category_id=j.job_category_id LEFT JOIN applications a ON a.job_id=j.job_id WHERE j.employer_user_id=? GROUP BY j.job_id ORDER BY j.created_at DESC");
...
<main class="dashboard">
    ...
    <section class="verification <?= e($profile['verification_status']) ?>">
        ...
        </div><?php if ($profile['verification_status'] !== 'approved'): ?><form method="post" enctype="multipart/form-data"><input type="hidden" name="action" value="document"><input type="file" name="criminal_record" accept=".pdf,.jpg,.jpeg,.png" required><button class="btn btn-primary">ส่งเอกสาร</button></form><?php else: ?><span class="status accepted">✓ ผ่านการยืนยัน</span><?php endif ?>
    </section>
    <section class="panel">
        <h2>ประกาศงานของคุณ <span class="count"><?= count($jobs) ?></span></h2><?php foreach ($jobs as $job): ?><div class="application">
                    <div><b><?= e($job['title']) ?></b>
                        <p><?= job_type($job['job_type']) ?> · <?= pay_text($job) ?></p><small>ผู้สมัคร <?= e((string)$job['applicants']) ?> คน · <?= $job['status'] === 'published' ? 'เผยแพร่แล้ว' : 'ปิดแล้ว' ?></small>
                    </div>
                    <div class="d-flex flex-wrap gap-2">
                        ...
                        <form method="post" action="<?= BASE_URL ?>/employer/jobdelete.php" onsubmit="return confirm('ยืนยันการลบประกาศงานนี้? ข้อมูลผู้สมัครของประกาศนี้จะถูกลบด้วย');"><input type="hidden" name="job_id" value="<?= $job['id'] ?>"><button class="btn btn-sm btn-outline-danger" type="submit">ลบ</button></form>
                    </div>
