...
$jobStmt = $pdo->prepare('SELECT j.job_id, j.job_title, j.job_description, j.work_location, j.work_province, j.work_schedule, j.pay_amount, j.pay_unit, j.open_positions, jc.category_slug FROM jobs j JOIN job_categories jc ON jc.job_category_id=j.job_category_id WHERE j.job_id=? AND j.employer_user_id=?');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $pdo->prepare('UPDATE jobs SET job_category_id=?, job_title=?, job_description=?, work_location=?, work_province=?, work_schedule=?, pay_amount=?, pay_unit=?, open_positions=? WHERE job_id=? AND employer_user_id=?')
            ->execute([
                $categoryId,
                trim($_POST['title'] ?? ''),
