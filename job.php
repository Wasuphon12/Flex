...
$stmt = db()->prepare("SELECT j.job_id AS id,j.job_title AS title,j.job_status AS status,jc.category_slug AS job_type,j.job_description AS description,j.work_location AS location,j.work_schedule AS work_date,j.pay_amount,j.pay_unit,j.open_positions AS positions,ep.company_name,ep.company_description,ep.company_logo_path AS company_logo,(SELECT ji.image_file_path FROM job_images ji WHERE ji.job_id=j.job_id ORDER BY ji.display_order, ji.job_image_id LIMIT 1) AS cover_image FROM jobs j JOIN employer_profiles ep ON ep.user_id=j.employer_user_id JOIN job_categories jc ON jc.job_category_id=j.job_category_id WHERE j.job_id=? AND (j.job_status='published' OR ?=1)");
        ...
        <article class="col-lg-11 col-xl-10">
            <div class="card border-0 shadow-sm overflow-hidden">
                ...
                <div class="card-body p-4 p-md-5">
                    ...
                    <p class="text-secondary mb-4"><?php if ($job['company_logo']): ?><img class="company-logo company-logo-detail" src="<?= BASE_URL . '/' . e($job['company_logo']) ?>" alt="โลโก้ <?= e($job['company_name']) ?>"><?php endif; ?><?= e($job['company_name']) ?> <span class="text-primary">✓ ผู้ว่าจ้างยืนยันแล้ว</span></p>
